## Running power flow without considering delay time off for the inverters
using Pkg
Pkg.activate("./")
using OpenDSSDirect
...
load_voltage_filter[:,1]=250*ones(87,1);
########
# MAIN LOOP
...
for i = 2:TT
 # defining the inverter at all load buses
    ...
    if  i>=200
       _ODSS.Vsources.PU(0.92*1.08) #.06,250. 0.07 255
     else
...
plot(Vmag_Phase_a[:,2:1000]')
plot(Vmag_Phase_b[:,2:1000]')
plot(Vmag_Phase_c[:,2:1000]')
